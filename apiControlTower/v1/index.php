    ...
    if (isset($headers['Api-Key']) && isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        if ($response1 == 'true' ) {
            ...
            $query= mysqli_query($conectar,"SELECT tl.transId,tl.profileId,tl.pageId,tl.startTime,tl.endTime,tl.startDate,tl.endDate,tl.totalTime,tl.isActive,pl.name as pageName,pl.urlPage FROM transmissionRecord tl JOIN generalPages pl ON pl.pageId=tl.pageId  where  tl.profileId='$modelId' and tl.startDate>='$sdate' and tl.endDate<='$edate' ORDER BY tl.startDate,tl.startDate DESC ");
